from Combine import *
def threes(shortfall,allocations1,spaces1,backupsize=5,used5=None,upperbound=10):
    if used5 is None:
        used5=set()
    allocations=[]
    spaces=[]
    ...
    x=combine(allocations,spaces,shortfall.copy(),backupsize,used5,upperbound)
    if x[1]:
        return x
                    ...
                    for j in range(len(spaces)-1, i+1, -1):
                        ...
                        for k in range(j-1, i, -1):
                            ...
                            if spaces[i]+spaces[j]+spaces[k]>=7 and (shortfall4[0]>0) and (i not in used4 and j not in used4 and k not in used4) and sum(allocations[i])+sum(allocations[k])+sum(allocations[j])<=bound5:
                                ...
                                trial=combine(allocations,spaces,shortfall4,backupsize,used4)
                                if trial[1]:
                    ...
                    for j in range(len(spaces)-1, i+1, -1):
                        ...
                        for k in range(j-1, i, -1):
                            ...
                            if spaces[i]+spaces[j]+spaces[k]>=6 and (shortfall5[1]>0) and (i not in used5 and j not in used5 and k not in used5) and sum(allocations[i])+sum(allocations[k])+sum(allocations[j])<=bound6:
                                ...
                                trial=combine(allocations,spaces,shortfall5,backupsize,used5)
                                if trial[1]:
...
def threes1(shortfall,allocations1,spaces1,backupsize=5,used5=None):
    if used5 is None:
        used5=set()
    allocations=[]
    spaces=[]
    ...
    x=combine1(allocations,spaces,shortfall.copy(),backupsize,used5)
    if x[1]:
        return x
    ...
    for i in range(0,len(spaces)-2):
        ...
        for j in range(len(spaces)-1,i+1,-1):
            ...
            for k in range(j-1,i,-1):
                if six6==0 and backup6==0:
                    break
                if (i not in used6 and j not in used6 and k not in used6) and spaces[i]+spaces[k]+spaces[j]>=(2*max(backupsize,6)) and (six6>0 or backup6>0) :
                    if spaces[i]+spaces[k]+spaces[j]>=(2*max(backupsize,6)):
                        if backupsize==7 and backup6>=2:
                            backup6-=2
                            used6.update([i,j,k])
                        ...
                        if six6==0 and backup6==0:
                            return threes6,init
                        trial=combine1(allocations,spaces,[backup6,six6],backupsize,used6)
                        if trial[1]:
                            return trial[0]+threes6,trial[1]+init
    six7=six6
    backup7=backup6
    ...
    for i in range(0,len(spaces)-2):
        ...
        for j in range(len(spaces)-1,i+1,-1):
            ...
            for k in range(j-1,i,-1):
                if six7==0 and backup7==0:
                    break
                if (i not in used7 and j not in used7 and k not in used7) and spaces[i]+spaces[k]+spaces[j]>=(max(backupsize,6)+min(backupsize,6)) and (six7>0 and backup7>0):
                    if spaces[i]+spaces[k]+spaces[j]>=(max(backupsize,6)+min(backupsize,6)):
                        if backup7>0 and six7>0:
                            backup7-=1
                            six7-=1
                        ...
                        if six7==0 and backup7==0:
                            return threes7,init1
                        trial=combine1(allocations,spaces,[backup7,six7],backupsize,used7)
                        if trial[1]:
                            return trial[0]+threes7,trial[1]+init1
    six8=six7
    backup8=backup7
    ...
    for i in range(0,len(spaces)-2):
        ...
        for j in range(len(spaces)-1,i+1,-1):
            ...
            for k in range(j-1,i,-1):
                if six8==0 and backup8==0:
                    break
                if (i not in used8 and j not in used8 and k not in used8) and spaces[i]+spaces[k]+spaces[j]>=(2*min(backupsize,6)) and (six8>0 or backup8>0):
                    if spaces[i]+spaces[k]+spaces[j]>=(2*min(backupsize,6)):
                        if backupsize==7 and six8>=2:
                            six8-=2
                            used8.update([i,j,k])
                        ...
                        if six8==0 and backup8==0:
                            return threes8,init2
                        trial=combine1(allocations,spaces,[backup8,six8],backupsize,used8)
                        if trial[1]:
                            return trial[0]+threes8,trial[1]+init2
            ...
            for i in range(len(spaces)-2):
                ...
                for j in range(len(spaces)-1, i+1, -1):
                    ...
                    for k in range(j-1, i, -1):
                        if shortfall4[0]==0:
                            break
                        if spaces[i]+spaces[j]+spaces[k]>=7 and (shortfall4[0]>0) and (i not in used4 and j not in used4 and k not in used4):
                            used4.add(i)
                            used4.add(j)
                            used4.add(k)
                            shortfall4[0]-=1
                            ...
                            if shortfall4[0]==0 and shortfall4[1]==0:
                                return triplecombos4,init3
                            trial=combine1(allocations,spaces,shortfall4,backupsize,used4)
                            if trial[1]:
                                return trial[0]+triplecombos4,trial[1]+init3
            shortfall5=shortfall4.copy()
            used5=used4.copy()
            ...
            for i in range(len(spaces)-2):
                ...
                for j in range(len(spaces)-1, i+1, -1):
                    ...
                    for k in range(j-1, i, -1):
                        if shortfall5[1]==0:
                            break
                        if spaces[i]+spaces[j]+spaces[k]>=6 and (shortfall5[1]>0) and (i not in used5 and j not in used5 and k not in used5):
                            used5.add(i)
                            used5.add(j)
                            used5.add(k)
                            shortfall5[1]-=1
                            ...
                            if shortfall5[0]==0 and shortfall5[1]==0:
                                return triplecombos5,init4
                            trial=combine1(allocations,spaces,shortfall5,backupsize,used5)
                            if trial[1]:
                                return trial[0]+triplecombos5,trial[1]+init4
        else:
            for i in range(len(spaces)-2):
                ...
                for j in range(len(spaces)-1, i+1, -1):
                    ...
                    for k in range(j-1, i, -1):
                        if shortfall4[1]==0:
                            break
                        if spaces[i]+spaces[j]+spaces[k]>=6 and (shortfall4[1]>0) and (i not in used4 and j not in used4 and k not in used4):
                            used4.add(i)
                            used4.add(j)
                            used4.add(k)
                            shortfall4[1]-=1
            ...
            for i in range(len(spaces)-2):
                ...
                for j in range(len(spaces)-1, i+1, -1):
                    ...
                    for k in range(j-1, i, -1):
                        if shortfall5[0]==0:
                            break
                        if spaces[i]+spaces[j]+spaces[k]>=5 and (shortfall5[0]>0) and (i not in used5 and j not in used5 and k not in used5):
                            used5.add(i)
                            used5.add(j)
                            used5.add(k)
                            shortfall5[0]-=1
